...
(define (enqueue x q)
  (if (>= (stream-length (queue-left q)) (+ 1 (length (queue-right q))))
      ...
      (make-queue (rotate (queue-left q) (cons x (queue-right q)) empty-stream) '() (+(+ (stream-length (queue-left q)) (length (queue-right q))) 1) 0)))
...
(define (dequeue q)
  (if (< (- (stream-length (queue-left q)) 1) (length (queue-right q)))
      (make-queue (rotate (stream-rest (queue-left q)) (queue-right q) empty-stream) '() (sub1 (+ (stream-length (queue-left q)) (length (queue-right q)))) 0)
      (make-queue (stream-rest (queue-left q)) (queue-right q) (sub1(queue-size-l q)) (queue-size-r q))))
