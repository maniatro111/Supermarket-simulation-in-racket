...
(define (tt+ C minutes)
  (match C [(counter index tt queue) (make-counter index (+ tt minutes) queue)]))
...
(define (min-tt-help counters acc)
  (if (null? counters) (cons (counter-index acc) (counter-tt acc))
      (if (< (counter-tt (car counters)) (counter-tt acc)) (min-tt-help (cdr counters) (car counters))
          (min-tt-help (cdr counters) acc))))
  ...
  (if (null? requests)
      ...
      (match (car requests)
        [(list 'delay index minutes) (cond
                                       ((eq? index 1) (serve (cdr requests) (tt+ C1 minutes) C2 C3 C4))
                                       ((eq? index 2) (serve (cdr requests) C1 (tt+ C2 minutes) C3 C4))
                                       ((eq? index 3) (serve (cdr requests) C1 C2 (tt+ C3 minutes) C4))
                                       (else (serve (cdr requests) C1 C2 C3 (tt+ C4 minutes))))]
        [(list name n-items) (cond
                               ((and (<= n-items ITEMS) (eq? 1 (car (min-tt (list C1 C2 C3 C4))))) (serve (cdr requests) (add-to-counter C1 name n-items) C2 C3 C4))
                               ((eq? 2 (car (min-tt (list C2 C3 C4)))) (serve (cdr requests) C1 (add-to-counter C2 name n-items) C3 C4))
                               ((eq? 3 (car (min-tt (list C2 C3 C4)))) (serve (cdr requests) C1 C2 (add-to-counter C3 name n-items) C4))
