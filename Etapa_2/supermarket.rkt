...
(define add-to-counter
  (λ (nume)
    (λ (produse)
      (λ (C)
        (if (null? (counter-queue C)) (make-counter (counter-index C) (counter-tt ((tt+ produse) C)) (counter-et ((et+ produse) C)) (append (counter-queue C) (list (cons nume produse))))
         (make-counter (counter-index C) (counter-tt ((tt+ produse) C)) (counter-et C) (append (counter-queue C) (list (cons nume produse)))))))))
...
(define rezolvare
  (λ (varianta)
    (λ (counters)
        (if (equal? varianta 'min-tt) (foldl (λ (element minim)
            ...
            (foldl (λ (element minim)
                     (if (or (and (< (counter-et element) (cdr minim)) (not(null? (counter-queue element)))) (and (eq? (car minim) -1) (not(null? (counter-queue element)))))
...
(define min-tt
  (rezolvare 'min-tt)) ; folosind funcția de mai sus
(define min-et
  (rezolvare 'min-et)) ; folosind funcția de mai sus
...
(define (remove-first-from-counter C)
  (make-counter (counter-index C) (foldl (λ (element acc) (+ acc (cdr element))) 0 (cdr (counter-queue C))) (if (null? (cdr (counter-queue C))) 0 (cdr(cadr(counter-queue C)))) (cdr (counter-queue C))))
...
(define (serve requests fast-counters slow-counters)
  ...
  (if (null? requests)
      ...
      (match (car requests)
        ...
        [(list 'delay index minutes) (serve (cdr requests) (update (et+ minutes) fast-counters index) (update (et+ minutes) slow-counters index))]
        [(list 'remove-first) (serve (cdr requests)
                                    (update remove-first-from-counter fast-counters (car (min-et(append fast-counters slow-counters))))
                                    (update remove-first-from-counter slow-counters (car (min-et(append fast-counters slow-counters)))))]
        [(list name items) (if (<= items ITEMS)
                              (serve (cdr requests) (update ((add-to-counter name) items) fast-counters (car (min-tt(append fast-counters slow-counters))))
                                    (update ((add-to-counter name) items) slow-counters (car (min-tt(append fast-counters slow-counters)))))
                              (serve (cdr requests) fast-counters (update ((add-to-counter name) items) slow-counters (car (min-tt slow-counters)))))]
        )))