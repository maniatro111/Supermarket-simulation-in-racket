...
; Veți întoarce coada actualizată.
(define (dequeue q)
  (if (null? (queue-left q)) (dequeue (make-queue (reverse (queue-right q)) '() (queue-size-r q) 0))
...
(define (top q)
  (if (null? (queue-left q)) (top (make-queue (reverse (queue-right q)) '() (queue-size-r q) 0))
     (car (queue-left q))))
